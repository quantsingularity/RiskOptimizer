...
def validate_portfolio_request(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(user_address, str) or not user_address.strip():
        raise ValidationError(
            "User address must be a non-empty string", "user_address", user_address
        )
    ...
    if not isinstance(allocations, dict):
        raise ValidationError(
            "Allocations must be a dictionary", "allocations", allocations
        )
    ...
    for asset, percentage in allocations.items():
        if not isinstance(asset, str) or not asset.strip():
            raise ValidationError(
                f"Invalid asset symbol: {asset}", "allocations", asset
            )

        if not isinstance(percentage, (int, float)):
            raise ValidationError(
                f"Percentage for {asset} must be a number", "allocations", percentage
            )

        if percentage < 0 or percentage > 100:
            raise ValidationError(
                f"Percentage for {asset} must be between 0 and 100",
                "allocations",
                percentage,
            )
    ...
    if "name" in data:
        ...
        if not isinstance(name, str) or not name.strip():
            raise ValidationError(
                "Portfolio name must be a non-empty string", "name", name
            )

    # Return validated data
    validated_data = {"user_address": user_address, "allocations": allocations}
...
def validate_portfolio_update_request(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "description" in data:
        ...
        if description is not None and not isinstance(description, str):
            raise ValidationError(
                "Portfolio description must be a string", "description", description
            )
    ...
    if "total_value" in data:
        ...
        if not isinstance(total_value, (int, float)):
            raise ValidationError(
                "Total value must be a number", "total_value", total_value
            )

        if total_value < 0:
            raise ValidationError(
                "Total value cannot be negative", "total_value", total_value
            )
