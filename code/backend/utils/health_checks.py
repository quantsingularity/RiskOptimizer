...
class HealthCheck:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            duration = time.time() - start_time
            logger.error(f"Health check failed for {self.name}: {str(e)}", exc_info=True)
...
class HealthCheckManager:
    ...
    def _create_summary(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        healthy_checks = len([r for r in results if r['status'] == HealthStatus.HEALTHY.value])
        degraded_checks = len([r for r in results if r['status'] == HealthStatus.DEGRADED.value])
        unhealthy_checks = len([r for r in results if r['status'] == HealthStatus.UNHEALTHY.value])
