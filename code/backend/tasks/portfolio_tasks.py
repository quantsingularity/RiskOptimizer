...
@task_with_progress()
def optimize_portfolio(self, assets_data: Dict, optimization_params: Dict) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if method not in ['mean_variance', 'risk_parity', 'black_litterman', 'minimum_variance']:
        ...
        asset_names = assets_data.get('asset_names', [f'Asset_{i}' for i in range(len(returns_df.columns))])
        ...
        if method == 'mean_variance':
            weights = _optimize_mean_variance(mean_returns, cov_matrix, optimization_params)
            ...
            weights = _optimize_black_litterman(mean_returns, cov_matrix, optimization_params)
        
        task_result_manager.store_task_progress(
            self.request.id,
            {'progress': 70, 'status': 'Calculating portfolio metrics'}
        )
        ...
        results = {
            ...
            'risk_contributions': {asset_names[i]: float(risk_contrib[i]) for i in range(len(risk_contrib))},
        ...
        logger.error(f"Portfolio optimization failed for task {self.request.id}: {str(e)}")
        raise
...
@task_with_progress()
def rebalance_portfolio(self, current_portfolio: Dict, target_weights: Dict, 
                       rebalancing_params: Dict) -> Dict[str, Any]:
    """
    ...
    try:
        task_result_manager.store_task_progress(
            self.request.id,
            {'progress': 20, 'status': 'Analyzing current portfolio'}
        )
        ...
        task_result_manager.store_task_progress(
            self.request.id,
            {'progress': 80, 'status': 'Calculating rebalancing costs'}
        )
        
        # Calculate rebalancing costs
        transaction_cost_rate = rebalancing_params.get('transaction_cost_rate', 0.001)  # 0.1%
        total_costs = total_turnover * transaction_cost_rate
        
        # Calculate tracking error reduction
        tracking_error_before = _calculate_tracking_error(current_weights, target_weights)
        ...
        results = {
            ...
            'rebalancing_analysis': {
                ...
                'tracking_error_reduction': tracking_error_before - tracking_error_after
            },
        ...
        logger.info(f"Portfolio rebalancing analysis completed for task {self.request.id}")
        return results
        
    except Exception as e:
        logger.error(f"Portfolio rebalancing analysis failed for task {self.request.id}: {str(e)}")
        raise
...
@task_with_progress()
def analyze_portfolio_performance(self, portfolio_data: Dict, benchmark_data: Dict, 
                                analysis_period: Dict) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        # Alpha calculation
        alpha = portfolio_annualized_return - (risk_free_rate + beta * (benchmark_annualized_return - risk_free_rate))
        
        task_result_manager.store_task_progress(
            self.request.id,
            {'progress': 80, 'status': 'Calculating drawdown metrics'}
        )
        ...
        results = {
            ...
            'benchmark_comparison': {
                ...
                'excess_return': float(portfolio_annualized_return - benchmark_annualized_return),
            ...
            'risk_metrics': {
                ...
                'downside_deviation': float(_calculate_downside_deviation(portfolio_returns)),
                'sortino_ratio': float(_calculate_sortino_ratio(portfolio_returns, risk_free_rate))
            },
            'metadata': {
        ...
        logger.info(f"Portfolio performance analysis completed for task {self.request.id}")
        return results
        
    except Exception as e:
        logger.error(f"Portfolio performance analysis failed for task {self.request.id}: {str(e)}")
        raise
