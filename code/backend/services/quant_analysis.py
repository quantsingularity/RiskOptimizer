...
class RiskMetrics:
    ...
    def efficient_frontier(returns, cov_matrix):
        ...
        try:
            ...
            raise ImportError("PyPortfolioOpt is not installed. Please install it using `pip install PyPortfolioOpt`.")
    ...
    def calculate_sharpe_ratio(returns, risk_free_rate=0.02):
        ...
        if not isinstance(returns, (list, np.ndarray)) or len(returns) < 2:
            raise ValueError("Returns must be a list or numpy array with at least two elements.")
