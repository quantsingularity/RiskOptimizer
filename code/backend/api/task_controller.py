...
async def start_monte_carlo_simulation(
    portfolio_data: Dict, num_simulations: int = 10000, time_horizon: int = 252
):
...
async def start_var_cvar_calculation(
    portfolio_data: Dict, confidence_levels: List[float] = [0.95, 0.99]
):
    """Start VaR/CVaR calculation task."""
    try:
        task = risk_tasks.calculate_var_cvar.delay(portfolio_data, confidence_levels)
...
async def start_efficient_frontier_calculation(
    assets_data: Dict, num_portfolios: int = 10000
):
...
async def start_stress_test(portfolio_data: Dict, stress_scenarios: List[Dict]):
    """Start portfolio stress test task."""
    try:
        task = risk_tasks.stress_test_portfolio.delay(portfolio_data, stress_scenarios)
...
async def start_portfolio_optimization(assets_data: Dict, optimization_params: Dict):
...
async def start_portfolio_rebalancing(
    current_portfolio: Dict, target_weights: Dict, rebalancing_params: Dict
):
...
async def start_performance_analysis(
    portfolio_data: Dict, benchmark_data: Dict, analysis_period: Dict
):
...
async def start_portfolio_report_generation(portfolio_data: Dict, report_config: Dict):
...
async def start_risk_report_generation(risk_analysis_data: Dict, report_config: Dict):
...
async def start_portfolio_data_export(portfolio_data: Dict, export_config: Dict):
    """Start portfolio data export task."""
    try:
        task = report_tasks.export_portfolio_data.delay(portfolio_data, export_config)
...
async def get_task_result(task_id: str):
    ...
    try:
        ...
        if task_result.failed():
            raise HTTPException(
                status_code=500, detail=f"Task failed: {task_result.info}"
            )
