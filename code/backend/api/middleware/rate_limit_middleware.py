        ...
        def wrapper(*args, **kwargs) -> Any:
            try:
                ...
                if len(count) >= requests_per_minute:
                    ...
                    if len(count) >= burst:
                        logger.warning(f"Rate limit exceeded for {endpoint}: {len(count)} requests in last minute")
                ...
                if isinstance(response, tuple) and len(response) == 2:
                    ...
                    if isinstance(response_obj, Response):
                        ...
                        response_obj.headers['X-RateLimit-Remaining'] = str(max(0, requests_per_minute - len(count)))
