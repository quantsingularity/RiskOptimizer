...
def validate_returns_data(returns: Any, field_name: str = "returns") -> List[float]:
    ...
    if len(returns) < 2:
        raise ValidationError(f"At least two data points are required in {field_name}", field_name, returns)
    
    # Validate each return value
    for i, value in enumerate(returns):
        if not isinstance(value, (int, float)):
            raise ValidationError(f"Return at index {i} is not a number: {value}", field_name, value)
...
def validate_efficient_frontier_request(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(returns, dict):
        raise ValidationError("Returns must be a dictionary mapping asset symbols to returns", "returns", returns)
    ...
    if 'min_weight' in data:
        ...
        if not isinstance(min_weight, (int, float)):
            raise ValidationError("Minimum weight must be a number", "min_weight", min_weight)
    ...
    if 'max_weight' in data:
        ...
        if not isinstance(max_weight, (int, float)):
            raise ValidationError("Maximum weight must be a number", "max_weight", max_weight)
    ...
    if 'points' in data:
        ...
        if not isinstance(points, int):
            raise ValidationError("Number of points must be an integer", "points", points)
        
        if points < 2:
            raise ValidationError("Number of points must be at least 2", "points", points)
        
        if points > 100:
            raise ValidationError("Number of points cannot exceed 100", "points", points)
