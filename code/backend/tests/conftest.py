    ...
    return [
        {
            ...
            'time_horizon': 252,
            'expected_metrics': ['var_95', 'var_99', 'cvar_95', 'cvar_99', 'probability_of_loss']
        },
        {
...
def performance_analysis_test_data():
    ...
    portfolio_returns = (alpha + beta * market_returns + 
...
def assert_risk_metrics_reasonable(risk_metrics):
    ...
    if 'var_99' in risk_metrics:
        ...
        if 'var_95' in risk_metrics:
            assert risk_metrics['var_99'] <= risk_metrics['var_95'], "VaR 99% should be more extreme than VaR 95%"
    
    # CVaR should be more extreme than VaR
    if 'cvar_95' in risk_metrics and 'var_95' in risk_metrics:
        assert risk_metrics['cvar_95'] <= risk_metrics['var_95'], "CVaR 95% should be more extreme than VaR 95%"
    
    if 'cvar_99' in risk_metrics and 'var_99' in risk_metrics:
        assert risk_metrics['cvar_99'] <= risk_metrics['var_99'], "CVaR 99% should be more extreme than VaR 99%"
...
def assert_performance_metrics_reasonable(performance_metrics):
    ...
    if 'total_return' in performance_metrics:
        assert -1 <= performance_metrics['total_return'] <= 10, "Total return outside reasonable range"
    
    if 'annualized_return' in performance_metrics:
        assert -1 <= performance_metrics['annualized_return'] <= 2, "Annualized return outside reasonable range"
    
    # Volatility should be positive and reasonable
    if 'volatility' in performance_metrics:
        assert 0 <= performance_metrics['volatility'] <= 2, "Volatility outside reasonable range"
    
    # Sharpe ratio should be reasonable
    if 'sharpe_ratio' in performance_metrics:
        assert -5 <= performance_metrics['sharpe_ratio'] <= 5, "Sharpe ratio outside reasonable range"
    
    # Max drawdown should be negative or zero
    if 'max_drawdown' in performance_metrics:
        assert performance_metrics['max_drawdown'] <= 0, "Max drawdown should be negative or zero"
...
def create_test_portfolio(n_assets=5, total_value=1000000):
    ...
    # Calculate holdings
    holdings = {name: weight * total_value for name, weight in zip(asset_names, weights)}
...
def validate_monte_carlo_result(result):
    ...
    required_keys = ['simulation_parameters', 'risk_metrics', 'statistics', 'percentiles']
    for key in required_keys:
        assert key in result, f"Missing key: {key}"
    
    # Validate risk metrics
    risk_metrics = result['risk_metrics']
    required_risk_metrics = ['var_95', 'var_99', 'cvar_95', 'cvar_99', 'probability_of_loss']
...
def validate_optimization_result(result):
    ...
    required_keys = ['optimization_method', 'weights', 'portfolio_metrics', 'constraints_satisfied']
    for key in required_keys:
        assert key in result, f"Missing key: {key}"
...
def validate_performance_analysis_result(result):
    ...
    required_benchmark_metrics = ['beta', 'alpha', 'tracking_error', 'information_ratio']
