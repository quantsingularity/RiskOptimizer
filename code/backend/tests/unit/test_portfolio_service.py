...
class TestPortfolioService(unittest.TestCase):
    ...
    def test_get_portfolio_by_address_success(self):
        ...
        mock_portfolio_data = {
            ...
            "allocations": [
                {"percentage": Decimal("50.0"), "amount": Decimal("500.0"), "current_price": Decimal("10.0")}
            ]
        }
        self.portfolio_service.portfolio_repo.get_portfolio_with_allocations.return_value = mock_portfolio_data
    ...
    def test_get_portfolio_by_address_not_found(self):
        ...
        self.portfolio_service.portfolio_repo.get_portfolio_with_allocations.side_effect = NotFoundError("Portfolio not found")
        self.portfolio_service.cache.get.return_value = None
    ...
    def test_save_portfolio_success(self):
        ...
        name = "My Portfolio"
        mock_portfolio_data = {"portfolio_id": 1, "user_address": user_address, "name": name}
        mock_user = MagicMock(id=1)

        self.portfolio_service.portfolio_repo.save_portfolio_with_allocations.return_value = mock_portfolio_data
    ...
    def test_create_portfolio_success(self):
        ...
        result = self.portfolio_service.create_portfolio(user_id, user_address, name, description)
    ...
    def test_delete_portfolio_success(self):
        portfolio_id = 1
        mock_portfolio = MagicMock(id=portfolio_id, user_id=1, user_address="test_address", name="Test Portfolio")
    ...
    def test_get_user_portfolios_success(self):
        user_id = 1
        mock_portfolios = [
            MagicMock(id=1, user_id=user_id, user_address="addr1", name="P1", description="D1", total_value=Decimal("100"), created_at=datetime.utcnow(), updated_at=datetime.utcnow()),
            MagicMock(id=2, user_id=user_id, user_address="addr2", name="P2", description="D2", total_value=Decimal("200"), created_at=datetime.utcnow(), updated_at=datetime.utcnow())
        ]
        self.portfolio_service.portfolio_repo.get_by_user_id.return_value = mock_portfolios
    ...
    def test_validate_portfolio_input_valid(self):
        ...
        self.portfolio_service._validate_portfolio_input(user_address, allocations, name)

    def test_validate_portfolio_input_invalid_address(self):
        with self.assertRaises(ValidationError):
            self.portfolio_service._validate_portfolio_input(None, {"BTC": 100.0}, "Name")

    def test_validate_portfolio_input_invalid_allocations(self):
        with self.assertRaises(ValidationError):
            self.portfolio_service._validate_portfolio_input("address", {}, "Name")
        with self.assertRaises(ValidationError):
            self.portfolio_service._validate_portfolio_input("address", {"BTC": 120.0}, "Name")
