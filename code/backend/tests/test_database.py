...
def test_get_portfolio_success(mock_execute_query):
    ...
    mock_address = "0x123"
    mock_data = [
        {"id": 1, "user_address": mock_address, "created_at": None, "asset_symbol": "ETH", "percentage": 0.6},
        {"id": 1, "user_address": mock_address, "created_at": None, "asset_symbol": "BTC", "percentage": 0.4}
    ]
...
def test_save_portfolio_new(mock_psycopg2):
    ...
    mock_cursor.execute.assert_any_call("SELECT id FROM portfolios WHERE user_address = %s", (user_address,))
    mock_cursor.execute.assert_any_call("INSERT INTO portfolios (user_address) VALUES (%s) RETURNING id", (user_address,))
    mock_cursor.execute.assert_any_call("INSERT INTO allocations (portfolio_id, asset_symbol, percentage) VALUES (%s, %s, %s)", (5, "AAPL", 0.7))
    mock_cursor.execute.assert_any_call("INSERT INTO allocations (portfolio_id, asset_symbol, percentage) VALUES (%s, %s, %s)", (5, "GOOG", 0.3))
    mock_conn.commit.assert_called_once()
...
def test_save_portfolio_update(mock_psycopg2):
    ...
    mock_cursor.execute.assert_any_call("SELECT id FROM portfolios WHERE user_address = %s", (user_address,))
    mock_cursor.execute.assert_any_call("DELETE FROM allocations WHERE portfolio_id = %s", (10,))
    mock_cursor.execute.assert_any_call("INSERT INTO allocations (portfolio_id, asset_symbol, percentage) VALUES (%s, %s, %s)", (10, "MSFT", 1.0))
    mock_conn.commit.assert_called_once()
