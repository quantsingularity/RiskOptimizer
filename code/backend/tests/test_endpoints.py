...
def test_root_endpoint(client) -> Any:
    ...
    with patch(
        "riskoptimizer.infrastructure.database.session.check_db_connection",
        return_value=True,
    ), patch(
        "riskoptimizer.infrastructure.cache.redis_cache.RedisCache.health_check",
        return_value=True,
    ):
        response = client.get("/")
        assert response.status_code == 200
        data = response.get_json()
        assert "name" in data or "RiskOptimizer" in str(data)
...
def test_login_invalid_credentials(client) -> Any:
    ...
    with patch(
        "riskoptimizer.domain.services.auth_service.auth_service.authenticate_user",
...
def test_var_endpoint_with_valid_data(client) -> Any:
    ...
    with patch(
        "riskoptimizer.api.middleware.auth_middleware.auth_service.verify_token",
        return_value={"user_id": 1, "email": "t@t.com", "role": "user"},
    ), patch(
        "riskoptimizer.api.middleware.auth_middleware.auth_service.is_token_blacklisted",
        return_value=False,
    ), patch(
        "riskoptimizer.domain.services.risk_service.risk_service.calculate_var",
...
def test_cvar_endpoint_with_valid_data(client) -> Any:
    ...
    with patch(
        ...
        "riskoptimizer.domain.services.risk_service.risk_service.calculate_cvar",
