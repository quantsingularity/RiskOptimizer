...
class ExtremeValueRisk:
    ...
    def fit_pot(self, data, threshold=None, threshold_quantile=0.1):
        ...
        if len(exceedances) < 10:
            ...
            try:
                ...
                # Log results
                logger.info(f"GPD fit: shape={shape:.4f}, scale={scale:.4f}, threshold={self.threshold:.4f}")
            except:
    ...
    def fit_block_maxima(self, data, block_size=20):
        ...
        if len(block_maxima) < 10:
            ...
            try:
                ...
                # Log results
                logger.info(f"GEV fit: shape={shape:.4f}, loc={loc:.4f}, scale={scale:.4f}")
            except:
        ...
        if method == 'evt':
            if self.pot_params is not None:
                ...
                if abs(shape) < 1e-10:  # Shape close to 0
                    ...
                    scenarios = threshold + (scale / shape) * ((u / severity_factor)**shape - 1)
                ...
                scenarios = stats.genextreme.rvs(shape, loc=loc, scale=scale, size=n_scenarios)
            ...
            if severity == 'extreme':
                ...
                if len(extreme_indices) > 0:
                    extreme_indices = np.random.choice(extreme_indices, n_scenarios // 2)
    ...
    def calculate_tail_dependence(self, x, y, method='empirical', threshold_quantile=0.1):
        """
    ...
    def plot_tail_distribution(self, confidence_levels=[0.9, 0.95, 0.99, 0.999]):
        ...
        ax.plot(x, stats.norm.pdf(x, np.mean(self.data), np.std(self.data)), 
        ...
        for i, conf in enumerate(confidence_levels):
            ...
            # Plot VaR
            ax.axvline(-var, color=color, linestyle='--', 
                      label=f'VaR ({conf*100:.1f}%): {var:.4f}')
            
            # Plot ES
            ax.axvline(-es, color=color, linestyle=':',
