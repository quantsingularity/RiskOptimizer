...
def historical_var(returns: pd.DataFrame, confidence_level: float = 0.99, horizon: int = 1) -> pd.Series:
    """
...
def monte_carlo_var(returns: pd.DataFrame, confidence_level: float = 0.99, horizon: int = 1, n_simulations: int = 10000) -> pd.Series:
    """
    ...
    for ticker in returns.columns:
        ...
        simulated_returns = np.random.normal(mu[ticker] * horizon, sigma[ticker] * np.sqrt(horizon), n_simulations)
