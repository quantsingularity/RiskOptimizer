...
class MLRiskModel:
    ...
    def predict_var(self, returns, confidence=0.95, feature_window=10):
        ...
        var_pred = np.maximum(
            var_pred, 0.001
        )  # Ensure positive VaR values with minimum threshold
...
class CopulaMLRiskModel:
    ...
    def generate_scenarios(self, n_scenarios=None):
        ...
        if self.copula_type == "gaussian":
            ...
            adjusted_corr = np.power(
                self.correlation_matrix, 1.5
...
class HybridRiskModel:
    ...
    def calculate_var(
        self, returns=None, weights=None, confidence=0.95, feature_window=10
    ):
        """
