...
contract("PortfolioTracker", (accounts) => {
  it("should update portfolio", async () => {
    ...
    await portfolioTrackerInstance.updatePortfolio(assets, allocations, { from: accounts[0] });
    ...
    assert.deepEqual(portfolio[1].map(a => a.toNumber()), allocations, "Allocations do not match");
  });
  ...
  it("should fail with invalid input", async () => {
    ...
    try {
      ...
      assert.include(error.message, "Invalid input", "Error message should contain 'Invalid input'");
    }
  });
  ...
  it("should fail if allocations do not sum to 10000", async () => {
    ...
    try {
      ...
      assert.include(error.message, "Allocations must sum to 100%", "Error message should contain 'Allocations must sum to 100%'");
    }
  });
});
