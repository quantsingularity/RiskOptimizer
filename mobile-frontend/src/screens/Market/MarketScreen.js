...
const MarketScreen = ({ navigation }) => {
    ...
    const fetchAssetDetails = useCallback(async (symbol) => {
        try {
            ...
            const previousClose = meta?.chartPreviousClose || (closes.length > 1 ? closes[closes.length - 2] : latestPrice);
            const change = previousClose !== 0 ? ((latestPrice - previousClose) / previousClose) * 100 : 0;
            return {
    ...
    useEffect(() => {
        if (selectedViewIndex === 0) { // Only search when in 'All' view
    ...
    const dataToDisplay = selectedViewIndex === 0
        ? (searchQuery.trim() !== '' ? searchResults : marketData)
        : watchlistData;
    ...
    const ListEmpty = () => (
        <View style={styles.centered}>
            {loading || (isSearching && selectedViewIndex === 0) ? (
                ...
                <Button title="Retry" onPress={selectedViewIndex === 0 ? fetchInitialMarketData : fetchWatchlistData} />
            ) : (
                <Text style={styles.infoText}>
                    {selectedViewIndex === 0
                        ? (searchQuery.trim() ? 'No assets found matching your search.' : 'No market data available.')
